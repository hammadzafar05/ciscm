...
@section('crumb')
    <span><a href="@route('success_story')">Success Story <i class="ion-ios-arrow-forward"></i></a></span>

    <span class="mr-2"><a href="#">@lang('default.post')</a></span>
@endsection
@section('content')
            ...
            <div class="row">
                          ...
                          <div class="col-md-12 col-lg-12 ftco-animate">
                                <div class="blog-entry">
                                    @if(!empty($blogPost->cover_photo))
                                        ...
                                        @endphp
                                    @else
                                        @php $image = 'img/ward.jpg'; @endphp
                                    @endif

                                    <a href="{{ route('success_story.story',['blogPost'=>$blogPost->id]) }}"
                                       class="block-20 d-flex align-items-end" @if(!empty($blogPost->cover_photo))
                                       style="background-image: url('{{ asset($image) }}');" @endif >
                                    ...
                                    <div class="text bg-white p-4">
                                        <h3 class="heading"><a href="{{ route('success_story.story',['blogPost'=>$blogPost->id]) }}">{{ $blogPost->title }}</a></h3>
                ...
                <div class="col-md-3">
                    <aside class="single_sidebar_widget search_widget">
                        <form method="get" action="{{ route('success_story') }}">
                    ...
                    <aside class="mt-4">
                        ...
                        <ul class="list-group">
                            ...
                            @foreach($categories as $category)
                                <li class="list-group-item @if(request()->get('category') == $category->id) active @endif"><a href="{{ route('success_story') }}?category={{ $category->id }}">{{ $category->name }}</a></li>
                            @endforeach
                    ...
                    <aside class="mt-4">
                        ...
                        <div class="ftco-footer-widget mb-5">
                            ...
                            @foreach(\App\BlogPost::whereDate('publish_date','<=',\Illuminate\Support\Carbon::now()->toDateTimeString())->where([
			['enabled', '1'],
			['show_at_success_stories', '1'],
		])->orderBy('publish_date','desc')->limit(5)->get() as $blogPost)
                                ...
                                <div class="block-21 mb-4 d-flex">
                                    ...
                                    @endif
                                        ...
                                        <a class="blog-img mr-4" style="background-image: url({{ asset($image) }});"></a>
                                    <div class="text">
                                        <h3 class="heading"><a href="{{ route('success_story.story',['blogPost'=>$blogPost->id]) }}">{{ $blogPost->title }}</a></h3>
                                        <div class="meta">
			...
			['show_at_success_stories', '1'],
                                ...
                                <div class="media post_item">
                                    ...
                                    @endif
                                        ...
                                        <img class="recent-blog-img" src="{{ asset($image) }}" alt="">
