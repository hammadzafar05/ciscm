...
class CourseController extends Controller {
    ...
    public function lecture(Request $request,Lecture $lecture,Course $course){
		...
		//dd($course->lessons()->orderBy('pivot_sort_order')->get());
		
		
	    $admin_id = Admin::where('id',$course->admin_id)->first();
	    ...
	    foreach($course->lessons()->orderBy('pivot_sort_order')->get() as $lesson){
		    foreach($lesson->lectures()->orderBy('sort_order')->get() as $classLecture){
			    $array_lecture_lists[] = $classLecture->id;
		    }
	    }
	    $key = array_search($lectureId, $array_lecture_lists);
		if ($key == 0){
			$previuos_lecture = FALSE;
		}else{
			$prev_key = $key - 1;
			$previuos_lecture = route(MODULE.'.course.lecture',['course'=>$sessionId,'lecture'=>$array_lecture_lists[$prev_key]]);
		}
	
	    $last_lecture = FALSE;
	    $lastKey = key(array_slice($array_lecture_lists, -1, 1, true));
		if($lastKey == $key){
			$last_lecture = route(MODULE.'.course.lecture',['course'=>$sessionId,'lecture'=>$array_lecture_lists[$lastKey]]);
		}
        ...
        $output = [
          'pageTitle'=>__lang('Class').': '.$lecture->name,
            ...
            'downloads'=>$downloads,
            'logo'=>$logo,
            'previuos_lecture'=>$previuos_lecture,
	    ...
	    $status = DB::select("
			SELECT *
			  FROM `lesson_access`
			 WHERE `lesson_id` ='".$lecture->lesson_id."'
			   AND `student_id`='".$studentId."'
			   AND `course_id` ='".$sessionId."'
		");
	    if($status){
		    $lesson_status = 'Limited';
	    }else{
		    $lesson_status = 'Full';
	    }
	    /*--MARUF END--*/
    ...
    private function checkLectureOrder($lectureId,$sessionId){
        ...
        if(!empty($lesson->enforce_lecture_order) && $lectureRow->sort_order > 1){
            ...
            if(!$sessionLogTable->hasAttendance($this->getId(),$sessionId,$previousLecture->id)){
                ...
                if($nextLecture){
                    redirect_now(route(MODULE.'.course.lecture',['course'=>$sessionId,'lecture'=>$nextLecture]));
                }
                else{
                      redirect_now(back()->getTargetUrl());
                }
    ...
    public function verifyClass($id,$session){
                ...
                if(!empty($lessonRow->test_required) && !empty($lessonRow->test_id) && $testTable->recordExists($lessonRow->test_id) && !$studentTestTable->passedTest($this->getId(),$lessonRow->test_id)){
                   ...
                   // flashMessage('You are required to take this test in order to complete the \''.$lessonRow->name.'\' class');
                    ...
                    if($nextClass){
	                    ...
	                    //return redirect()->route(MODULE.'.course.lecture',['lecture'=>$nextRow->id,'course'=>$nextClass->course_id]);
                    }
                    else{
                        //classes are over
