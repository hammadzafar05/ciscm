...
class SessionController extends Controller
{
        ...
        if(request()->isMethod('post')){
            ...
            if($form->isValid()){
                ...
                if($error == 0) {
	                ...
	                if ($admin_role == 'Partner'){
		                $message = 'New course request [<b>'.$data['session_name'].'</b>] from Partner: '.adminName(Auth::user()->id).'<br><br>';
		                $this->sendEmail('info@worldacademy.uk','New Course Request from Partner: '.$data['session_name'],$message);
	                }
        ...
        if(request()->isMethod('post')){
            ...
            if($form->isValid()){
	            ...
	            if($error == 0) {
		            ...
		            if (isset($formData['session_instructor_id'])) {
			            //dd($formData['session_instructor_id']);
			            $course->admins()->sync($formData['session_instructor_id']);
						//dd($course->admins());
			           
		            }
		
		            session()->flash('flash_message', __lang('Changes Updated!'));
	...
	/*--MARUF START--*/
    public function manage_class(Request $request,$id)
    {
        $output = [];
        $studentSessionTable = new StudentSessionTable();
        ...
        $lesson_status = array();
	    $totalLessonsDetails = $sessionLessonTable->getSessionRecords($row->course_id);
	    foreach ($totalLessonsDetails as $view){
		    /*ManageStudentLessonAccess::where([
			    ['name', '=', 'custom'],
		    ...
		    $status = DB::select("
						SELECT *
						  FROM `lesson_access`
						 WHERE `lesson_id` ='".$view->lesson_id."'
						   AND `student_id`='".$output['student_id']."'
						   AND `course_id` ='".$view->course_id."'
			");
		    if($status){
			    $lesson_status[$view->lesson_id] = 'Limited';
		    }else{
			    $lesson_status[$view->lesson_id] = 'Full';
		    }
	    }
	    
 
	    $output['lesson_status'] = $lesson_status;
	    $output['totalLessonsDetails'] = $totalLessonsDetails;
        ...
        $output['testResults'] = $testResults;
        $output['pageTitle'] = __lang('student-manage-class').': '.$row->name.' '.$row->last_name;

        $output['customCrumbs'] = [
            route('admin.dashboard')=>__('default.dashboard'),
            adminUrl(['controller'=>'student','action'=>'sessions'])=>__lang('session-courses'),
            adminUrl(['controller'=>'student','action'=>'sessionstudents','id'=>$row->course_id])=>__lang('Enrolled Students'),
            '#'=>__lang('student-manage-class')
        ];


        return view('admin.session.manage_access',$output);
    }
    public function limit_lesson_access(Request $request){
	...
	public function result_publications(Request $request,$id)
	{
		...
		if(request()->isMethod('post')){
			...
			DB::table('student_courses')
				...
				->update([
					...
					'result_grade' => $request->input('result_grade'),
					'result_cgpa' => $request->input('result_cgpa'),
					'result_certificate_number' => $request->input('result_certificate_number'),
			...
			$output['student_courses_id'] = $id;
			$output['row'] = $row;
			$output['student_id'] = $row->student_id;

			
			$output['pageTitle'] = __lang('student_result_publication') . ': ' . $row->name . ' ' . $row->last_name;
			...
			return view('admin.session.result_publications', $output);
		}
	}
    ...
    public function smssession(Request $request){
        ...
        if(request()->isMethod('post'))
        {
	        ...
	        $sms_schedule_time = $request->post('schedule_time');
	        $sms_schedule_type = $request->post('sms_schedule_type');
	        $course_id = $request->post('sms_course_id');
	        $students = $request->post('sms_students');
	        $type = $request->post('sms_type');
	        $sms_student_by_courses = $request->post('sms_student_by_courses');
	        if ($type == 'Course'){
		        if ($sms_student_by_courses == null){
			        $students_by = $studentSessionTable->getSessionRecords(false,$course_id,true);
			        foreach ($students_by as $student) {
				        $sms_student_by_courses[] = $student->mobile_number;
			        }
		        }
	        }
	
	        if ($sms_student_by_courses){
		        foreach ($sms_student_by_courses as $key => $val){
					if ($val != null) {
						$students[] = $val;
					}
		        }
			...
			if ($sms_schedule_type == "Scheduled"){
				foreach ($numbers as $number) {
					...
					$data = ScheduleSms::create($dbData);
				}
				session()->flash('flash_message', 'Scheduled SMS saved successfully!');
