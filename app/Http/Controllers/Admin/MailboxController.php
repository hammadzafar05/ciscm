...
use App\Model\MailboxFolder;
use App\Model\Mailbox;
use App\Model\MailboxAttachment;
use App\Model\MailboxFlags;
use App\Model\MailboxReceiver;
use App\Model\MailboxTmpReceiver;
use App\Model\MailboxUserFolder;
use App\User;
...
class MailboxController extends Controller
{
    ...
    public function index(Request $request, $folder = "")
    {
	    ...
	    $pageTitle = 'Mailbox';
        return view('pages.mailbox.index', compact('folders', 'messages', 'unreadMessages','pageTitle'));
    }
    ...
    public function create()
    {
	    ...
	    if ($admin_role == 'Partner'){
		    $users = User::where('enabled', 1)->where('id', '!=', Auth::user()->id)->whereIn('id', [1])->get();
	    }else {
		    $partners = array();
			$partners[] = 0;
		    $admins = User::where('id', '!=', Auth::user()->id)->whereHas('admin')->get();
			foreach($admins as $admin){
				if ($admin->admin->adminRole->name == 'Partner') {
					$partners[] = $admin->id;
				}
			}
			
		    $users = User::where('enabled', 1)->where('id', '!=', Auth::user()->id)->whereIn('id', $partners)->get();
	    }
     
	    
		$pageTitle = 'Compose Email';
        return view('pages.mailbox.compose', compact('folders', 'unreadMessages', 'users','pageTitle'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $mailbox->body = $body;
        $mailbox->receiver_id = $receiver_ids;
...
//        $folder = $mailbox->userFolder()->folder()->first();
	    ...
	    $pageTitle = 'Show Email';
        return view('pages.mailbox.show', compact('folders', 'unreadMessages', 'mailbox','pageTitle'));
    }
    ...
    public function getReply($id)
    {
	    ...
	    $pageTitle = 'Reply';
        return view('pages.mailbox.reply', compact('folders', 'unreadMessages', 'mailbox','pageTitle'));
    }
    ...
    public function postReply(Request $request, $id)
    {
        ...
        $mailbox->receiver_id = $old_mailbox->sender_id; // NEW LINE
        $mailbox->sender_id = Auth::user()->id;
        $mailbox->time_sent = date("Y-m-d H:i:s");
    ...
    public function getForward($id)
    {
        ...
        $users = User::where('enabled', 1)->where('id', '!=', Auth::user()->id)->whereIn('id', [727,226,1,729])->get();
	
	    $pageTitle = 'Forward';
        return view('pages.mailbox.forward', compact('folders', 'unreadMessages', 'mailbox', 'users','pageTitle'));
    }
