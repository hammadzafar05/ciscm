...
class LectureController extends Controller
{
    ...
    public function addfile(Request $request,$id){
        ...
        ->where('student_lectures.lecture_id', '=', $id)
            ->select('users.email')
            ->get()->toArray();
        dispatch(new SendMultipleEmails($data,$id));
...
// The PDF source is in original.pdf
	...
	public function addPowerPoint(Request $request,$id){
		$table = new LecturePageTable();
		if(request()->isMethod('post')){
			$formData = request()->all();
			$form = $this->getPowerPointForm();
			$form->setData($formData);
			...
			if($form->isValid()){
				...
				try{
					...
					if($request->file())
					{
						...
						$destinationPath = public_path('uploads/lecture_content');
						$uploadedFile->move($destinationPath,$filename);
						$data['lecture_content_file'] = !empty($filename) ? $filename:null;
					}
					unset($data['path']);
					...
					$table->addRecord([
						...
						'title'=>$data['title'],
						'content'=>$data['content'],
						'lecture_content_file'=> !empty($filename) ? $filename:null,
						'sort_order'=>$data['sort_order'],
						'type'=>'p'
					]);
					$table->arrangeSortOrders($id);
...
// BASE_PATH and BASE_URL set elsewhere in the script
	...
	private function getPowerPointForm(){
		$form = new BaseForm();
		$form->createHidden('path');
		$form->get('path')->setAttribute('id','path');
		$form->createText('title','Title',true);
		$form->createTextArea('instructions','Instructions');
		$form->createText('sort_order','Sort Order');
		$form->setInputFilter($this->getPowerPointFilter());
		return $form;
	}
	
	private function getPowerPointFilter(){
		$filter = new InputFilter();
		$filter->add([
			'name'=>'title',
			'required'=>true,
