...
class StudentController extends Controller
{
    ...
    {
	    $has_permissions_to_add_student = check_route_permissions('admin.student.add');
	    $has_permissions_to_delete_student = check_route_permissions('admin.student.delete');
	    ...
	    if ($admin_role == 'Partner'){
		    $total = User::whereIn('id',$enrolled_users)->count();
		    $paginator = $table->getStudents(true,$filter,$enrolled_users);
	    }else {
        ...
        return viewModel('admin',__CLASS__,__FUNCTION__,array(
            ...
            'filter'=>$filter,
            'text'=>$text,
            'has_permissions_to_add_student'=>$has_permissions_to_add_student,
            'has_permissions_to_delete_student'=>$has_permissions_to_delete_student,
    ...
    public function add(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $data = $request->all();
			$country = $request->input('custom_2');
	        ...
	        /*----Starting WU-81 : add student using country dropdown not working ----*/
	        //dd($form->isValid());
            //if ($form->isValid()  && !$studentsTable->emailExists($data['email']) ) {
            if (!$studentsTable->emailExists($data['email']) ) {

                //$data = $form->getData();
	        /*----ending WU-81 : add student using country dropdown not working ----*/
...
/*                $studentTable = new StudentTable();
    ...
    public function sessions(Request $request){
        ...
        $options =[''=>'--'.__lang('Select Category').'--'];
        ...
        return view('admin.student.sessions',array(
            'paginator'=>$paginator,
            'pageTitle'=>__lang('courses-and-sessions').'  ('.$paginator->getTotalItemCount().')',
            'attendanceTable'=>$attendanceTable,
            ...
            if($form->isValid()){
	            ...
	            if ($data['payment_required'] == "1"){
		            if ($data['amount'] == ''){
			            ...
			            $output['flash_message'] = 'Course Fee is required';
		            ...
		            $data['emi_status'] = 'Disabled';
		            $data['emi_installment'] = '0';
	            }
	            ...
	            if($error == 0) {
		            ...
		            $dbData = array(
			            ...
			            'emi_status' => $data['emi_status'],
			            'emi_installment' => $data['emi_installment'],
			            'admin_id' => $this->getAdmin()->admin->id
		            );
		            $admin_role = getAdminRole(Auth::user()->id);
		            if ($admin_role == 'Partner'){
			            $dbData['approved_by_super_admin'] = 'Pending';
		            }
    ...
    public function editsession(Request $request,$id){
        ...
        if($request->isMethod('post')){
            ...
            if($form->isValid()){
				...
				if (array_key_exists('session_instructor_id',$formData)) {
					foreach ($formData['session_instructor_id'] as $value) {
						$groupId = $value[0];
						$formData['session_instructor_id[]'][] = $groupId;
					}
            ...
            $data['emi_status']=$row->emi_status;
            $data['emi_installment']=$row->emi_installment;
			
	        foreach($sessionRow->courseCategories as $groupRow){
...
/*            $rowset = $sessionLessonTable->getSessionRecords($id);
	...
	/*--MARUF START--*/
	public function generate_batch(Request $request,$id,$copy){
		...
		//create row
		$sessionArray= getObjectProperties($sessionRow);
		$parent_batch = $sessionArray['id'];
		unset($sessionArray['id']);
		$batch_number = 'NULL';
		if ($sessionArray['parent_batch'] == 0){
			$batch_number = '2';
			$sessionArray['batch_number'] = $batch_number;
		}else{
			$training_name = $sessionArray['name'];
			$training_name = explode(' - Batch ',$training_name);
			$batch_number  = @$training_name[1] + 1;
			$sessionArray['name'] = $training_name[0];
			$sessionArray['batch_number'] = $batch_number;
		}
		$sessionArray['name'] = $sessionArray['name'].' - Batch '.$batch_number;
		$sessionArray['parent_batch'] = $parent_batch;
		$newId = $sessionTable->addRecord($sessionArray);
	...
	public function question_bank_lists(Request $request,$id){
		...
		$classes = $sessionClassTable->getSessionRecords($id);

		
		$questionBankTable = new QuestionBankTable();
		$paginator = $questionBankTable->getPaginatedRecords(true,$id);
		...
		return viewModel('admin',__CLASS__,__FUNCTION__,array(
			'pageTitle'=>$sessionRow->name.' :  Questions ('.$questionBankTable->getTotalQuestions($id).')',
	...
	public function question_bank_import(Request $request,$id){
		$input_requests = $request->all();
		if ($input_requests['class_id'] == ''){
			session()->flash('flash_error_message', 'Class id is required');
			return back();
		}else {
			
			$testQuestionTable = new QuestionBankTable();
			...
			if (request()->isMethod('post')) {
				...
				foreach ($all_rows as $value) {
					...
					//create new question
					$testQuestion = new QuestionBank();
					$testQuestion->course_id = $input_requests['course_id'];
					$testQuestion->class_id = $input_requests['class_id'];
					$testQuestion->questions_type = $input_requests['questions_type'];
					...
					$testQuestion->save();
					$imported++;
					if ($input_requests['questions_type'] == 'MCQ') {
						//get options
						...
						foreach ($optionEntries as $optionValue) {
							...
							if (!empty($optionValue)) {
								$count++;
								$testOption = new QuestionBankOption();
								/*
								...
								*/
								$optionData = [
									...
									'option'=> trim($optionValue)
								];
								
								if ($count == $correctOption) {
									$optionData['is_correct'] = 1;
								}
								$testOption->addRecord($optionData);
							}
	...
	public function question_bank_delete(Request $request,$id,$qid)
	{
		$table = new QuestionBankTable();
		$row = $table->getRecord($qid);
		try{
			$table->deleteRecord($qid);
			flashMessage(__lang('Record deleted'));
		}
		catch(\Exception $ex){
			$this->deleteError();
    ...
    public function enroll(Request $request,$id)
    {
        ...
        $select->setAttribute('class','form-control select2insidemodal');
    ...
    public function export(Request $request,$id){
        ...
        for($i=1;$i<=$totalPages;$i++){
            ...
            foreach ($paginator as $row){
	            $std = User::find($row->user_id);
	            $reg_year = date('y-m',strtotime($std->created_at));
	            $reg_number = 'WA-'.$reg_year.'-'.str_pad($row->user_id,4,"0",STR_PAD_LEFT);
				
                $csvData = array($row->id,$row->name.' '.$row->last_name,$row->mobile_number,$row->email,$reg_number);
    ...
    public function export_results(Request $request,$id){
	    ...
	    $columns[] = 'RESULT_GRADE';
	    $columns[] = 'RESULT_CGPA';
	    $columns[] = 'RESULT_DESCRIPTION';
        ...
        for($i=1;$i<=$totalPages;$i++){
            ...
            foreach ($paginator as $row){
	            ...
	            $csvData[] = $sessionRow->name;/*COURSE_NAME*/
	            $csvData[] = $course_id;/*COURSE_ID*/
	            $csvData[] = $row->student_id;/*STUDENT_ID*/
	
	            $result_cgpa = $result_grade = $result_description =  $result_certificate_number =  $result_passing_year = '';
	            ...
	            if(sizeof($output_cgpa) > 0) {
		            ...
		            $result_description = @$output_cgpa[0]->result_description;
		            $result_certificate_number = @$output_cgpa[0]->result_certificate_number;
		            $result_passing_year = @$output_cgpa[0]->result_passing_year;
	            }
	
	
	            $csvData[] = $result_grade;/*RESULT_GRADE*/
	            $csvData[] = $result_cgpa;/*RESULT_CGPA*/
	            $csvData[] = $result_description;/*RESULT_DESCRIPTION*/
	            $csvData[] = $result_certificate_number;/*CERTIFICATE_NUMBER*/
	            $csvData[] = $result_passing_year;/*PASSING_YEAR*/
        ...
        $sessionName = $sessionRow->name;
        // It will be called downloaded.pdf
        header('Content-Disposition: attachment; filename="'.$sessionName.' Students for Importing Results - '.date('d/M/Y').'.csv"');
    ...
    public function exportbulkattendance(Request $request,$id){
        ...
        for($i=1;$i<=$totalPages;$i++){
            ...
            foreach ($paginator as $row){
	            ...
	            $csvData = array($row->student_id,$row->name.' '.$row->last_name,$row->mobile_number,$row->email,$reg_number);
        ...
        if($request->isMethod('post'))
        {
	        ...
	        if ($admin_role == 'Partner'){
		        foreach ($all_rows as $value) {
			        ...
			        $total++;
			        $dbData = array();
			        $dbData['course_id'] = $post['course_id'];
			        ...
			        $dbData['3_Designation'] = $value['3_Designation'];
			        $dbData['4_Organization'] = $value['4_Organization'];
			        $dbData['2_Country'] = $value['2_Country'];
			        $dbData['partner_id'] = Auth::user()->id;
			        $dbData['status'] = 'Pending';
			
			        if (empty($dbData['last_name']) || empty($dbData['email'])) {
				        continue;
			        }
			        $user = PartnerStudent::create($dbData);
		        }
		
				if ($total > 0) {
					$message = 'New enrollment request from '.Auth::user()->name.' ['.Auth::user()->email.']';
					$this->sendEmail('info@worldacademy.uk', 'New enrollment request from Partner', $message);
				}
				
		        $output['flash_message'] = __lang("your-enrollment_request-total", ['total' => $total]);
		        if (!empty($failure)) {
			        $output['flash_message'] .= " $failure " . __lang('records-failed');
		        }
	        }else {
		        ...
		        foreach ($all_rows as $value) {
			        ...
			        if (!$studentTable->emailExists($dbData['email'])) {
				        ...
				        $student = $studentTable->getStudentIdWithDetails($row->user_id);
				        $enrollData = array('student_id' => $student->id, 'course_id' => $sessionId);
				        if ($studentSessionTable->enrolled($student->id, $sessionId)) {
					        $failure++;
				        } else {
					        $total++;
					        $studentSessionTable->addRecord($enrollData);
	...
	public function student_partner_request()
	{
		$logged_in_user_id = Auth::user()->id;
		$admin_role = getAdminRole($logged_in_user_id);
		if ($admin_role == 'Partner'){
			$students  = PartnerStudent::where([
				['partner_id', '=', $logged_in_user_id],
			])->get();
		}else{
			$students  = PartnerStudent::get();
		}
		foreach ($students as $student){
		...
		$output['admin_role'] = $admin_role;
		$output['students'] = $students;
		$output['pageTitle'] = 'Student Enrollment Request By Partner';
		return view('admin.student.student_enrollment_request',$output);
	}
		...
		if ($table) {
			...
			if ($data) {
				...
				if ($value == 'Approve'){
					...
					$dbData['mobile_number'] = $data->mobile_number;
					...
					if (!$studentTable->emailExists($dbData['email'])) {
						...
						$enrollData = array('student_id' => $studentId, 'course_id' => $sessionId, 'reg_code' => $code);
						$studentSessionTable->addRecord($enrollData);
						...
						$this->sendEmail($dbData['email'],__lang('Enrollment Complete: '.$course->name),$emailMessage);
						
						$partner_details = User::find($partner_id);
						$partner_email = $partner_details->email;
						$message = __lang('enrollment-mail-partner',['student_name'=>$dbData['last_name'],'student_email'=>$dbData['email'],'course'=>$course->name,'code'=>$code]);
						$this->sendEmail($partner_email,__lang($dbData['last_name'].' Enrollment Complete: '.$course->name),$message);
						...
						$code = generateRandomString(5);
						$enrollData = array('student_id' => $student->id, 'course_id' => $sessionId, 'reg_code' => $code);
						if ($studentSessionTable->enrolled($student->id, $sessionId)) {
						...
						$message = __lang('enrollment-mail',['course'=>$course->name,'code'=>$code]);
						$emailMessage = $message.setting('regis_enroll_mail');
						$this->sendEmail($student_email,__lang('Enrollment Complete: '.$course->name),$emailMessage);
						
						$message = __lang('enrollment-mail-partner',['student_name'=>$dbData['last_name'],'student_email'=>$student_email,'course'=>$course->name,'code'=>$code]);
						$this->sendEmail($partner_email,__lang($dbData['last_name'].' Enrollment Complete: '.$course->name),$message);
					}
				}
    ...
    public function mailsession(Request $request,$id=null){
	    ...
	    $sessions = $sessionTable->getPaginatedRecords(FALSE,null,true,null,null,'asc',['s','b','c'],false);
	    
	    foreach ($sessions as $row) {
		    $students_by = $studentSessionTable->getSessionRecords(false,$row->id,true);
		    foreach ($students_by as $student) {
			    $student_by_courses[] = array('mobile_number'=>$student->mobile_number,'email'=>$student->email,'name'=>$student->name.' '.$student->last_name,'session_id'=>$row->id);
		    }
			if ($students_by->count() > 0) {
				$courses[] = array('id' => $row->id, 'name' => $row->name.'( Total Student: '.$students_by->count().')');
			}
		    /*$courses[] = array('id' => $row->id, 'name' => $row->name.'('.$students_by->count().')');*/
	    }
		
		
        if(!empty($id)){
	        $stds = $studentSessionTable->getSessionRecords(true,$id,true);
        ...
        if($request->isMethod('post'))
        {
	        ...
	        $email_schedule_time = $request->post('schedule_time');
	        $email_schedule_type = $request->post('email_schedule_type');
	        ...
	        if ($type == 'Course'){
		        if ($student_by_courses == null){
			        $students_by = $studentSessionTable->getSessionRecords(false,$course_id,true);
			        foreach ($students_by as $student) {
				        $student_by_courses[] = $student->email;
			        }
		        }
	        }
			
			if ($student_by_courses){
				foreach ($student_by_courses as $key => $val){
					$students[] = $val;
				}
			}
	        ...
	        if ($email_schedule_type == "Scheduled"){
				$emails = array();
		        if ($students) {
			        ...
			        for ($i = 1; $i <= $totalPages; $i++) {
				        ...
				        foreach ($paginator as $row) {
					        $emails[$row->email] = $row->name.' '.$row->last_name;
					        $count++;
				        }
			        }
		        }
		        ...
		        foreach ($emails as $key => $val) {
			        ...
			        $data = ScheduleEmail::create($dbData);
		        }
		        session()->flash('flash_message',__lang('schedule-email-total',['total'=>$count]));
	        }else {
		        if ($students) {
			        $rowsPerPage = 3000;
			        ...
			        for ($i = 1; $i <= $totalPages; $i++) {
				        foreach ($students as $key => $val) {
					        $this->sendEmail($val, $subject, $message, null, $senderName, $senderEmail);
					        $count++;
				        }
			        }
		        } else {
    ...
    public function invoicesByPartner(Request $request) {
        // TODO Auto-generated TransactionController::index(Request $request) default action
	    $partner_user_id = Auth::id();
	    $partner_admin_details = Admin::where('user_id', $partner_user_id)->first();
	    $partner_admin_id = $partner_admin_details->id;

        $this->data['pageTitle'] = 'Invoices for Partner';
        $paginator = PartnerInvoice::where('partner_admin_id',$partner_admin_id)->orderBy('id','desc')->paginate(20);
		foreach ($paginator as $page){
			$students = array();
			$student_ids = explode(',',$page->students_ids);
			$count = 0;
			foreach ($student_ids as $key => $id){
				$count++;
				$student = Student::where('id',$id)->first();
				$user = User::where('id',$student->user_id)->first();
				$students[] = $count.'. '.$user->name.' '.$user->last_name.' ('.$user->email.')';
			}
			$page->students = implode('<br>',$students);
		}
        $this->data['paginator'] = $paginator;

        return viewModel('admin','_partner',__FUNCTION__,$this->data);
    }
	
    public function viewInvoicesByPartner($id, Request $request) {

	    $output = array();
	    $invoice = PartnerInvoice::where('id',$id)->first();
		
	    $students = array();
	    $student_ids = explode(',',$invoice->students_ids);
	    $count = 0;
	    foreach ($student_ids as $key => $id){
		    $count++;
		    $student = Student::where('id',$id)->first();
	    ...
	    $invoice->students = implode('<br>',$students);
		
	    $output['invoice'] = $invoice;
     
	    return view('admin.partner.view_invoice',$output);
    }
	
    public function updateInvoicesByPartner($id, Request $request) {
	    $invoice = PartnerInvoice::where('id',$id)->first();
		if ($invoice){
			...
			$this->sendEmail('info@worldacademy.uk','Invoice Paid','Your Invoice #'.$invoice->invoice_number.' has been paid successfully.');
	...
	public function approvepartnertransaction(Request $request,$id){
    ...
    public function partnerInvoices(Request $request) {
	    ...
	    $admin_partner_ids[] = 0;
	    $user_partner_ids[] = 0;
	    $admin_partners = Admin::where('admin_role_id', 5)->get();
	    foreach($admin_partners as $admin){
		    $admin->user_details = User::where('enabled', 1)->where('id', '=', $admin->user_id)->first();
		    $admin_partner_ids[] = $admin->id;
		    $user_partner_ids[$admin->id] = $admin->user_id;
	    }
	    $this->data['partners'] = $admin_partners;

        $this->data['pageTitle'] = 'Partner Invoices';
        $paginator = PartnerInvoice::orderBy('id','desc')->paginate(20);
        $this->data['paginator'] = $paginator;
        $this->data['courses'] = Course::orderBy('name')->limit(5000)->get();
	    $this->data['currencies'] = Currency::get();
	    ...
	    $student_by_courses = array();
	    $sessionTable = new SessionTable();
	    $sessions = $sessionTable->getPartnerWiseSessions(FALSE,implode(',',$admin_partner_ids),true,null,null,'asc',null,false);
	    foreach ($sessions as $session) {
		    $students_by = $studentSessionTable->getSessionRecords(false, $session->id,true);
		
		    $invoiced_student_ids[] = 0;
		    $billed_student_ids = PartnerInvoice::where('course_id', $session->id)->first();
			if ($billed_student_ids){
				$invoiced_student_ids = $billed_student_ids->students_ids;
				$invoiced_student_ids = explode(',',$invoiced_student_ids);
			}
		    $invoiced_student_ids = array_unique($invoiced_student_ids);
			
		    foreach ($students_by as $student) {
				if(in_array($student->student_id,$invoiced_student_ids) == FALSE) {
					$student_by_courses[] = array(
		    ...
		    if ($students_by->count() > 0) {
			    $courses[] = array(
				    ...
				    'user_id' => $user_partner_ids[$session->admin_id]
			    );
		    }
	    }
		
	    $this->data['courses'] = $courses;
	    $this->data['student_by_courses'] = $student_by_courses;


        return viewModel('admin','_partner',__FUNCTION__,$this->data);
    }
	
	public function createPartnerInvoice(Request $request){
		...
		$data['students_ids'] = implode(',',$request->students_ids);
		$data['sent_date'] = date('Y-m-d H:i:s');
		$data['admin_user_id'] = Auth::id();
		$admin_admin_id = Admin::where('user_id', $data['admin_user_id'])->first();
		if ($admin_admin_id) {
			$data['admin_admin_id'] = $admin_admin_id->id;
		}else{
			$data['admin_admin_id'] = 0;
		}
		$data['cart'] = '';
		$invoice = PartnerInvoice::create($data);
		if ($invoice->status=='paid'){
			...
			$url= route('admin.partner.invoices');
	...
	public function deletePartnerInvoice(Request $request,$id){
    ...
    public function application_form_download(Request $request) {
        // TODO Auto-generated TransactionController::index(Request $request) default action
	
	    //$this->data['paginator']= ApplicationForm::orderBy('id','desc')->paginate(20);
	    $this->data['paginator']= ApplicationForm::orderBy('id','desc')->get();
        $this->data['pageTitle'] = __lang('application_form');
    ...
    public function approvetransaction(Request $request,$id){
        ...
        //$this->logPayment($id);
        //enroll student
//        $this->enrollStudent($row->student_id,$row->course_id);
	...
	/*--MARUF START--*/
	public function resend_invoice(Request $request,$id){
		...
		try{
			...
			session()->flash('flash_message',__lang('shomvabona-invoice-resend-success'));
		}
		catch(\Exception $ex){
			session()->flash('flash_message',__lang('shomvabona-invoice-resend-error'));
		}
    ...
    private  function  enrollStudent($studentId,$sessionId){
        ...
        // $this->sendEmail($student->email,__lang('Enrollment Complete'),$emailMessage);
            ...
            $messageData =[
                    'email'=>$student['email'],
                    'name'=>$student['name'],
                    'course_name'=>$sessionRow->name,
                    'code'=>$code,
                ];
                Mail::send('emails.enroll_course',$messageData,function($message) use ($email){
...
/*
	...
	public function import_external_certificates(Request $request){
		...
		if($request->isMethod('post'))
		{
			...
			foreach($all_rows as $value){
				$dbData = array();
				$dbData['course']=$value['program_name'];
				$dbData['title'] = $value['student_name'];
				$dbData['issue_date']=$value['issue_date'];
				$dbData['tracking_number']=$value['certificate_number'];
		...
		$output['pageTitle']=('Import External Certificates');
		
		return view('admin.student.import_external_certificates',$output);
	}
	...
	public function import_results(Request $request){
		...
		if($request->isMethod('post'))
		{
			...
			foreach($all_rows as $value){
				...
				if (($value['RESULT_CGPA'] != '') && ($value['RESULT_GRADE'] != '') && ($value['CERTIFICATE_NUMBER'] != '') && ($value['PASSING_YEAR'] != '')) {
					$dbData = array();
					
					
					$dbData['COURSE_ID'] = $value['COURSE_ID'];
					$dbData['STUDENT_ID'] = $value['STUDENT_ID'];
					$dbData['RESULT_GRADE'] = $value['RESULT_GRADE'];
					$dbData['RESULT_CGPA'] = $value['RESULT_CGPA'];
					$dbData['RESULT_DESCRIPTION'] = $value['RESULT_DESCRIPTION'];
					$dbData['CERTIFICATE_NUMBER'] = $value['CERTIFICATE_NUMBER'];
					$dbData['PASSING_YEAR'] = $value['PASSING_YEAR'];
					...
					try {
						...
						DB::table('student_courses')
							...
							->update([
								'result_description' => $dbData['RESULT_DESCRIPTION'],
								'result_cgpa' => $dbData['RESULT_CGPA'],
								'result_grade' => $dbData['RESULT_GRADE'],
								'result_certificate_number' => $dbData['CERTIFICATE_NUMBER'],
								'result_passing_year' => $dbData['PASSING_YEAR'],
							]);
					} catch (\Exception $ex) {
						$failure++;
					}
			...
			$msg = __lang("import-success",['total'=>$total]);
			if(!empty($failure)){
				$msg = " $failure ".__lang("records failed");
			}
			
		}
		//route('admin.student.sessionstudents',['id'=>$row->id])
		return redirect()->route('admin.student.sessionstudents', ['id' => $course_id])->with('flash_message', $msg);
	}
	...
	public function certificateBadge(Request $request,$id){
	   $course=Course::find($id);
	   $pageTitle ="Add Certificate For Badge For the ".$course->name;
	   if($request->isMethod('post')){
	        $data=$request->all();
	        ...
	        if($request->hasFile('certificate_image')){
                ...
                $request->certificate_image->move('usermedia/certificateandbadge/',$fileName);
                $course['certificate_image']=$fileName;
            }  
	        ...
	        if($request->hasFile('badge_image')){
                ...
                $fileName=time().'.'.$file->getClientOriginalExtension();
                $request->badge_image->move('usermedia/certificateandbadge/',$fileName);
                $course['badge_image']=$fileName;
            }
            
            $course->save();
            session()->flash('flash_message','Data Store Successfully !');
	   }
	   
	   return view('admin.student.append.create',compact('course','pageTitle'));
	}
}
